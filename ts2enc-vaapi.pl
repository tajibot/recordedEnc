...
#my $vaapi_param3 = "-c:v h264_vaapi -profile 100 -level 40";
#my $vaapi_param3 = "-c:v h264_vaapi -level 40";
#my $vaapi_param3 = "-c:v h264_vaapi -level 40 -segment_format_options movflags=+faststart";
...
if ( exists $opts{'two'} ) {
    ...
    # 1-pass処理
#    $trans_cmd  = "$ffmpeg_cmd $ffmpeg_front_opt $vaapi_param1 -i \"$input_file\" -vf '${vaapi_param2}${size}' -f mp4 $vaapi_param3 -qp $quality -threads $thread_num -aspect $aspect_param -acodec $acodec_param $multi_track $vf_opt $af_opt \"$output_file\"";
    $trans_cmd  = "$ffmpeg_cmd $opt0 $ffmpeg_front_opt $vaapi_param1 -i \"$input_file\" $filter -vf '${vaapi_param2}${size}' -f mp4 $vaapi_param3 -qp $quality -threads $thread_num -aspect $aspect_param -acodec $acodec_param $multi_track $vf_opt $af_opt \"$output_file\"";
